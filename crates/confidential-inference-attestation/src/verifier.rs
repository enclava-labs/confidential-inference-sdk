...
use crate::{
    ...
    FailClosedTinfoilQuoteVerifier, FixtureEvidence, GpuAttestationVerifier,
    IonetConfidentialEvidence, ModelBindingResult, NvidiaGpuAttestationEvidence,
    NvidiaGpuAttestationVerificationRequest, ParsedTinfoilLiveCapture, ReferenceValuesPayload,
    ResponseIntegrityResult, Result, RouteAttribution, RoutePartyAttribution, RoutePartyRole,
    SignatureMetadata, TcbComposeHashBinding, TinfoilLiveCaptureEvidence,
    TinfoilQuoteVerificationRequest, TinfoilQuoteVerifier, TinfoilTlsEvidence, TrustTier,
    ValidityWindow, VerdictArtifacts, VerdictError, VerificationStatus, VerifiedTinfoilQuote,
};
use base64::Engine;
...
pub fn verify_evidence_with_attestation_verifiers(
    ...
    match request.route.evidence_family.as_str() {
        ...
        "ionet_confidential" => verify_ionet_confidential_evidence_with_gpu_attestation_verifier(
            request,
            gpu_attestation_verifier,
        ),
...
pub fn verify_ionet_confidential_evidence(
    request: VerificationRequest,
) -> Result<AttestationVerdict> {
...
pub fn verify_ionet_confidential_evidence_with_gpu_attestation_verifier(
    request: VerificationRequest,
    gpu_attestation_verifier: &dyn GpuAttestationVerifier,
) -> Result<AttestationVerdict> {
    let evidence: IonetConfidentialEvidence = serde_json::from_slice(&request.raw_evidence)?;
    if evidence.schema != IonetConfidentialEvidence::SCHEMA {
        return Err(AttestationError::InvalidEvidence(format!(
            "unsupported schema {}",
            evidence.schema
        )));
    ...
    let cpu_verified = match &request.policy.hardware.cpu {
        ...
        CpuTeeRequirement::AnyCpuTee | CpuTeeRequirement::OneOf { .. } => {
            push_check(
                ...
                "cpu_tee",
                false,
                "io.net CPU quote verification backend is not configured",
            );
            false
        }
    };
    ...
    let gpu_verified = match &request.policy.hardware.gpu {
        ...
        GpuTeeRequirement::OneOf { allowed } => {
            let gpu_check = verify_ionet_gpu_tee(
                &evidence,
                allowed,
                gpu_attestation_verifier,
                &request.route.provider,
    ...
    let hardware_verified = cpu_verified && gpu_verified;
    checks.insert("tls_binding".into(), CheckResult::NotApplicable);
    checks.insert("e2ee_key_binding".into(), CheckResult::NotApplicable);

    let nonce_prefix_verified = !evidence.nonce_prefix.is_empty()
        && is_lower_hex(&evidence.nonce_prefix)
        && evidence.nonce.starts_with(&evidence.nonce_prefix)
        && is_lower_hex(&evidence.nonce);
    let freshness_nonce_matches = freshness_nonce_matches_policy(
        &request.policy,
        request.expected_freshness_nonce.as_deref(),
        &evidence.nonce_prefix,
    );
    let nonce_verified = nonce_prefix_verified && freshness_nonce_matches;
    push_check(
        &mut checks,
        &mut errors,
        "nonce_binding",
        nonce_verified,
        if freshness_nonce_matches {
            "io.net attestation nonce is not bound to the request nonce prefix"
        } else {
            "io.net attestation nonce prefix does not match expected per-request freshness nonce"
        },
    );

    let signing_address_digest = sha256_digest(evidence.signing_address.as_bytes());
    let response_signing_key_digest = route_reference.response_signing_key_digest.as_deref();
    let signing_key_bound = response_signing_key_digest == Some(signing_address_digest.as_str());
    let signing_key_binding_error = if response_signing_key_digest.is_some() {
        "io.net attested signing address does not match response signing key reference values"
    } else {
        "io.net attested signing address is not pinned by reference values"
    };
    push_check(
        &mut checks,
        &mut errors,
        ...
        signing_key_bound,
        signing_key_binding_error,
    );

    let image_reference_matches = evidence.image_digest == route_reference.workload_image_digest
        && evidence.workload_image_digest == route_reference.workload_image_digest;
    let response_receipt_verified = nonce_verified && signing_key_bound && image_reference_matches;
    push_check(
        &mut checks,
        &mut errors,
        "response_receipt",
        response_receipt_verified,
        "io.net response receipt key or image digest is not bound to reference values",
    );
    ...
    let response_integrity_result = if response_receipt_verified {
    ...
    let status = match request.policy.enforcement {
        EnforcementMode::Disabled => VerificationStatus::Disabled,
        _ if failed_required => VerificationStatus::Failed,
        _ if hardware_verified && response_receipt_verified => VerificationStatus::Verified,
        _ => VerificationStatus::Partial,
    };
    ...
    let verdict = AttestationVerdict {
        ...
        route_attribution: Some(build_route_attribution(
            &request,
            Some(evidence.gpu_tee.as_policy_str()),
        )),
        policy_schema: VerificationPolicy::SCHEMA.into(),
        reference_values_schema: ReferenceValuesPayload::SCHEMA.into(),
        provider_registry_schema: "confidential-inference.provider-registry.v1".into(),
        ...
        artifacts: VerdictArtifacts {
            ...
            report_data: Some(evidence.nonce),
            signing_public_key: Some(signing_address_digest),
...
fn verify_ionet_gpu_tee(
    evidence: &IonetConfidentialEvidence,
    allowed: &[GpuTeeKind],
    gpu_attestation_verifier: &dyn GpuAttestationVerifier,
    provider: &str,
    route_id: &str,
    ...
    if !allowed
        .iter()
        .any(|candidate| candidate == &evidence.gpu_tee)
    {
        return Err("GPU TEE kind is not accepted by policy".into());
    }

    match evidence.gpu_tee {
        GpuTeeKind::NvidiaCc => {
            let gpu_attestation = evidence
                .gpu_attestation
                .as_ref()
            ...
            if gpu_attestation.nonce != evidence.nonce {
                return Err("NVIDIA GPU attestation nonce does not match io.net nonce".into());
            }
...
mod tests {
    use super::*;
    use crate::{
        AliasConfidence, AttestedRoute, BoundDataRequirement, ChannelBindingKind,
        CpuTeeRequirement, FreshnessClass, GpuAttestationVerifier, GpuTeeKind, GpuTeeRequirement,
        IonetConfidentialEvidence, NvidiaGpuAttestationEvidence,
    ...
    fn ionet_route() -> AttestedRoute {
        AttestedRoute {
            provider: "ionet-fixture".into(),
            route_id: "ionet-fixture:llama-3.3-70b:ionet-model-llama-3-3-70b".into(),
            evidence_family: "ionet_confidential".into(),
            requested_model: "llama-3.3-70b".into(),
            provider_model: "ionet-model-llama-3-3-70b".into(),
            canonical_model: "llama-3.3-70b".into(),
            api_endpoint: "https://api.intelligence.io.net/v1/private".into(),
            evidence_endpoint: "https://api.intelligence.io.net/v1/private/attestation".into(),
            adapter_version: "ionet-confidential-fixture-adapter/0.1.0".into(),
            freshness_class: FreshnessClass::PerSession,
    ...
    fn request_with_ionet_evidence(evidence: IonetConfidentialEvidence) -> VerificationRequest {
        let mut providers = BTreeMap::new();
        let route = ionet_route();
        let route_reference = RouteReference {
            canonical_model: route.canonical_model.clone(),
            provider_model: route.provider_model.clone(),
            evidence_family: route.evidence_family.clone(),
            ...
            e2ee_public_key_digest: String::new(),
            response_signing_key_digest: Some(sha256_digest(IONET_SIGNING_ADDRESS.as_bytes())),
            tls_spki_sha256: None,
            workload_images: Vec::new(),
            workload_image_digest: "sha256:ionet-workload-image".into(),
            model_artifacts: vec![ArtifactDigest {
                kind: "provider_model".into(),
                name: "ionet-model-llama-3-3-70b".into(),
                digest: sha256_digest("ionet-model-llama-3-3-70b".as_bytes()),
            }],
            valid_until: "2099-01-01T00:00:00Z".into(),
            valid_until_epoch_ms: 4_070_908_800_000,
        };
        ...
        let reference_values = ReferenceValuesPayload {
            schema: ReferenceValuesPayload::SCHEMA.into(),
            version: "2026-07-05-ionet-fixture".into(),
            issuer: "confidential-inference".into(),
            valid_from: "2026-07-05T00:00:00Z".into(),
            valid_until: "2099-01-01T00:00:00Z".into(),
            valid_until_epoch_ms: 4_070_908_800_000,
            revocation_epoch: 1,
            minimum_acceptable_version: "2026-07-05-ionet-fixture".into(),
            providers,
        };
        let reference_values_digest = reference_values.digest().unwrap();
        let registry_digest = sha256_digest(b"ionet-registry");
        ...
        VerificationRequest {
            ...
            reference_values_source: "test".into(),
            registry_digest,
            registry_version: "2026-07-05-ionet-fixture".into(),
            registry_source: "test".into(),
            registry_sync_completed_at: "2026-07-05T00:00:00Z".into(),
            registry_signature: SignatureMetadata {
                signer: "confidential-inference".into(),
    ...
    const IONET_SIGNING_ADDRESS: &str = "0x1111111111111111111111111111111111111111";

    fn ionet_evidence() -> IonetConfidentialEvidence {
        let nonce_prefix = "44".repeat(16);
        let nonce = format!("{nonce_prefix}{}", "00".repeat(16));
        IonetConfidentialEvidence {
            ...
            nonce: nonce.clone(),
            nonce_prefix,
            signing_address: IONET_SIGNING_ADDRESS.into(),
            image_digest: "sha256:ionet-workload-image".into(),
            gpu_tee: GpuTeeKind::NvidiaCc,
            gpu_attestation: Some(nvidia_gpu_attestation(&nonce)),
            cpu_quote_sha256: Some("sha256:ionet-cpu-quote".into()),
    ...
    impl GpuAttestationVerifier for StaticIonetGpuAttestationVerifier {
        fn verify_nvidia_gpu_attestation(
            ...
            assert_eq!(request.expected_tee, GpuTeeKind::NvidiaCc);
            assert_eq!(request.expected_nonce, request.evidence.nonce);
            assert_eq!(request.provider, "ionet-fixture");
            assert_eq!(
                request.route_id,
                "ionet-fixture:llama-3.3-70b:ionet-model-llama-3-3-70b"
            );
            Ok(VerifiedGpuAttestation::nvidia_cc(
                request.expected_nonce,
                request.evidence.attestation_format.clone(),
                "static-ionet-test-verifier",
            ))
        }
    }
    ...
    #[test]
    fn ionet_confidential_evidence_verifies_receipt_bound_response_identity() {
        let request = request_with_ionet_evidence(ionet_evidence());

        let verdict = verify_ionet_confidential_evidence_with_gpu_attestation_verifier(
            request,
            &StaticIonetGpuAttestationVerifier,
        )
        .unwrap();
        ...
        assert_eq!(verdict.check("nonce_binding"), Some(&CheckResult::Verified));
        assert_eq!(
            verdict.check("response_signing_key_binding"),
            Some(&CheckResult::Verified)
        );
        assert_eq!(
            verdict.check("response_receipt"),
    ...
    #[test]
    fn ionet_per_request_freshness_accepts_verified_nonce_receipt() {
        let evidence = ionet_evidence();
        let expected_nonce = evidence.nonce_prefix.clone();
        let mut request = request_with_ionet_evidence(evidence);
        request.policy.freshness = FreshnessPolicy::PerRequest;
        request.expected_freshness_nonce = Some(expected_nonce);
    ...
    #[test]
    fn ionet_per_request_freshness_rejects_unexpected_nonce_prefix() {
        let evidence = ionet_evidence();
        let mut request = request_with_ionet_evidence(evidence);
        request.policy.freshness = FreshnessPolicy::PerRequest;
        request.expected_freshness_nonce = Some("55".repeat(16));
        ...
        assert!(verdict.errors.iter().any(|error| {
            error.code == "nonce_binding"
                && error
                    .message
                    .contains("expected per-request freshness nonce")
        }));
    }

    #[test]
    fn ionet_gpu_attestation_does_not_substitute_for_required_cpu_tee() {
        let mut request = request_with_ionet_evidence(ionet_evidence());
        request.policy.hardware.cpu = CpuTeeRequirement::AnyCpuTee;
        ...
        assert!(!verdict.request_allowed);
        assert_eq!(verdict.check("gpu_tee"), Some(&CheckResult::Verified));
        assert_eq!(verdict.check("cpu_tee"), Some(&CheckResult::Failed));
        assert!(verdict.errors.iter().any(|error| {
            error.code == "cpu_tee"
                && error
                    .message
                    .contains("io.net CPU quote verification backend is not configured")
        }));
    }

    #[test]
    fn ionet_confidential_evidence_fails_closed_without_response_signing_key_pin() {
        let mut request = request_with_ionet_evidence(ionet_evidence());
        request
            .reference_values
            .providers
    ...
    #[test]
    fn ionet_confidential_evidence_fails_closed_for_nonce_mismatch() {
        let mut evidence = ionet_evidence();
        evidence.nonce = "55".repeat(32);
        let request = request_with_ionet_evidence(evidence);
